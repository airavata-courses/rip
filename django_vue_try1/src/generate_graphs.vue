<template>
  <div>
    <button v-on:click="createChart()">Greet</button>

    <div id="app">
      <canvas id="planet-chart">{{planetChartData}}</canvas>
    </div>

  </div>

<!--<h1>{{planetChartData}}</h1>-->
</template>
      ...
      methods: {
        ...
        createChart() {
          ...
          for(var i = 0;i<jsonobj.length;i++)
          {
            ...
            var ans = jsonobj[i]['data'];

            var j;
            for (j = 0; j < ans.length; j++) {
                wav.push(ans[j].WaveLength);
                y.push(ans[j].Y);
            }
          ...
          // {
          //   chartData.data.datasets[0].data = result[i];
          //   chartData.data.labels = result[i-1];
          ...
          for(let k=0;i<result1.length;k+2)
          {
            console.log("inside loop");
            this.chartData.data[1].push({
            label: 'test'+k,
            data :result1[k],
